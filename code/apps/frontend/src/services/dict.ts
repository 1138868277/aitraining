...
/** 快捷搜索：根据数据码名称模糊匹配 */
export async function quickSearchDict(q: string, pageNum: number = 1, pageSize: number = 20, typeFilter?: string, secondClassFilter?: string, dataCategoryFilter?: string): Promise<{
  items: Array<{
    typeCode: string;
  ...
  const res = await api.get(`/dict/quick-search`, { params: { q, pageNum, pageSize, typeFilter, secondClassFilter, dataCategoryFilter } });
  return res.data;
}
