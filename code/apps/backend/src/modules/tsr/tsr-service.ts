...
const overallConfig: Record<string, { fields: string[]; headers: Record<string, string>; mergeCols: string[] }> = {
  ...
  tb: {
    fields: ['standard_name', 'tb_windows', 'sliding_step', 'begin_time', 'end_time', 'measure_name', 'cd_code'],
    headers: { standard_name: '标准化名称', tb_windows: '窗口大小(秒)', sliding_step: '滑动步长(秒)', begin_time: '生效开始时间', end_time: '生效结束时间', measure_name: '描述', cd_code: '组合31位码' },
    mergeCols: ['standard_name', 'tb_windows', 'sliding_step', 'begin_time', 'end_time', 'measure_name'],
  },
  yx: {
